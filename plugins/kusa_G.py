...
from utils import intToFormattedStr as fstr
...
async def _(session: CommandSession):
    ...
    st = f'您本周期的G市交易总结：\n'
    st += f'当前持仓相当于{fstr(nowKusaInG)}草，本周期共投入{fstr(allCostKusa)}草，共取出{fstr(allGainKusa)}草。\n'
    st += f'本周期盈亏估值：{fstr(nowKusaInG + allGainKusa - allCostKusa)}草。\n\n'
    ...
    if not (eastGAmount or southGAmount or northGAmount or zhuhaiGAmount or shenzhenGAmount):
        ...
        st += f'您的具体持仓如下：\n'
        st += (f'东校区： {eastGAmount}G * {gValues.eastValue:.3f} = {fstr(eValue)}草\n' if eastGAmount else '')
        st += (f'南校区： {southGAmount}G * {gValues.southValue:.3f} = {fstr(sValue)}草\n' if southGAmount else '')
        st += (f'北校区： {northGAmount}G * {gValues.northValue:.3f} = {fstr(nValue)}草\n' if northGAmount else '')
        st += (f'珠海校区： {zhuhaiGAmount}G * {gValues.zhuhaiValue:.3f} = {fstr(zValue)}草\n' if zhuhaiGAmount else '')
        st += (f'深圳校区： {shenzhenGAmount}G * {gValues.shenzhenValue:.3f} = {fstr(szValue)}草\n' if shenzhenGAmount else '')
    ...
    if isBuyingAll:
        ...
        for schoolName in '东南北珠深':
            ...
            if await buying(userId, gType, buyingAmount, totalPrice, 'G市(买)'):
                st += f'花费{fstr(totalPrice)}草，买入了{buyingAmount}{gType}\n'
        st = st.strip()
        if not st:
            st = '你不够草^ ^'
    elif buyingAmount:
        for schoolName in '东南北珠深':
            ...
            if await buying(userId, gType, buyingAmount * ratio, totalPrice, 'G市(买)'):
                st += f'花费{fstr(totalPrice)}草，买入了{buyingAmount * ratio}{gType}\n'
        st = st.strip()
        if not st:
            st = '你不够草^ ^'
    else:
...
async def _(session: CommandSession):
    ...
    if isSellingAll:
        if not schoolRatio:
            ...
            await session.send(f'已卖出所有G，获得了{fstr(allKusa)}草')
            return
        ...
        for schoolName in '东南北珠深':
            ...
            if await selling(userId, gType, sellingAmount * ratio, totalPrice, 'G市(卖)'):
                st += f'卖出了{sellingAmount}{gType}，获得了{fstr(totalPrice)}草\n'
        st = st.strip()
        if not st:
            st = '你没有可卖出的G^ ^'
    elif sellingAmount:
        for schoolName in '东南北珠深':
            ...
            if await selling(userId, gType, sellingAmount * ratio, totalPrice, 'G市(卖)'):
                st += f'卖出了{sellingAmount * ratio}{gType}，获得了{fstr(totalPrice)}草\n'
        st = st.strip()
        if not st:
            st = '你不够G^ ^'
    else:
